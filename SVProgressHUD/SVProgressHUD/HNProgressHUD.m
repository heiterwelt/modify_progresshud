...
#error HNProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "HNProgressHUD.h"
#import "HNIndefiniteAnimatedView.h"
#import "HNProgressAnimatedView.h"
...
NSString * const HNProgressHUDDidReceiveTouchEventNotification = @"HNProgressHUDDidReceiveTouchEventNotification";
NSString * const HNProgressHUDDidTouchDownInsideNotification = @"HNProgressHUDDidTouchDownInsideNotification";
NSString * const HNProgressHUDWillDisappearNotification = @"HNProgressHUDWillDisappearNotification";
NSString * const HNProgressHUDDidDisappearNotification = @"HNProgressHUDDidDisappearNotification";
NSString * const HNProgressHUDWillAppearNotification = @"HNProgressHUDWillAppearNotification";
NSString * const HNProgressHUDDidAppearNotification = @"HNProgressHUDDidAppearNotification";

NSString * const HNProgressHUDStatusUserInfoKey = @"HNProgressHUDStatusUserInfoKey";

static const CGFloat HNProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat HNProgressHUDUndefinedProgress = -1;
static const CGFloat HNProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat HNProgressHUDVerticalSpacing = 12.0f;
static const CGFloat HNProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat HNProgressHUDLabelSpacing = 8.0f;


@interface HNProgressHUD ()

@property (nonatomic, strong) NSTimer *fadeOutTimer;

@property (nonatomic, strong) UIControl *controlView;
@property (nonatomic, strong) UIView *backgroundView;
@property (nonatomic, strong) HNRadialGradientLayer *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) HNProgressAnimatedView *ringView;
@property (nonatomic, strong) HNProgressAnimatedView *backgroundRingView;
...
+ (HNProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static HNProgressHUD *sharedView;
#if !defined(HN_APP_EXTENSIONS)
...
+ (void)setDefaultStyle:(HNProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
    [self sharedView].hudView.alpha = style != HNProgressHUDStyleCustom ? 1.0f : 0.0f;
}

+ (void)setDefaultMaskType:(HNProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(HNProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:HNProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:HNProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(HNProgressHUDMaskType)maskType {
    HNProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = HNProgressHUDMaskTypeNone;
        _defaultStyle = HNProgressHUDStyleLight;
        _defaultAnimationType = HNProgressHUDAnimationTypeFlat;
        _minimumSize = CGSizeZero;
        _font = [UIFont preferredFontForTextStyle:UIFontTextStyleSubheadline];
        
        NSBundle *bundle = [NSBundle bundleForClass:[HNProgressHUD class]];
        NSURL *url = [bundle URLForResource:@"HNProgressHUD" withExtension:@"bundle"];
        ...
        _fadeInAnimationDuration = HNProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = HNProgressHUDDefaultAnimationDuration;
        
        _maxSupportedWindowLevel = UIWindowLevelNormal;
        
        // Accessibility support
        self.accessibilityIdentifier = @"HNProgressHUD";
        self.accessibilityLabel = @"HNProgressHUD";
...
- (void)updateHUDFrame {
    ...
    hudWidth = HNProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + HNProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = HNProgressHUDVerticalSpacing + labelHeight + contentHeight + HNProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += HNProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(HNProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - HNProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != HNProgressHUDUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + HNProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-HNProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(HNProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-HNProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(HNProgressHUDParallaxDepthPoints);
...
#else
            // If HNProgressHUD ist used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{HNProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
#if !defined(HN_APP_EXTENSIONS) && TARGET_OS_IOS
    self.frame = [[[UIApplication sharedApplication] delegate] window].bounds;
    UIInterfaceOrientation orientation = UIApplication.sharedApplication.statusBarOrientation;
#elif !defined(HN_APP_EXTENSIONS) && !TARGET_OS_IOS
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:HNProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:HNProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak HNProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong HNProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = HNProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)showStatus:(NSString*)status {
    ...
    if(self.defaultMaskType != HNProgressHUDMaskTypeNone) {
    ...
    if(self.hudView.alpha != 1.0f){
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HNProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        if (self.defaultStyle != HNProgressHUDStyleCustom && !greateriOS9) {
            [self addBlur];
        }
#endif
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.hudView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:HNProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
            }
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(HNProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:HNProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(strongSelf.hudView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:HNProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == HNProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[HNIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[HNIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        HNIndefiniteAnimatedView *indefiniteAnimatedView = (HNIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (HNProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[HNProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (HNProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[HNProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == HNProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == HNProgressHUDStyleDark) {
...
- (UIColor*)backgroundColorForStyle {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
    // On iOS 8 and HNProgressHUDStyleLight / HNProgressHUDStyleDark the
    // the background color is set via a UIVisualEffectHNiew
    return self.defaultStyle == HNProgressHUDStyleCustom ? self.backgroundColor : [UIColor clearColor];
#else
    if(self.defaultStyle == HNProgressHUDStyleLight) {
        return [UIColor whiteColor];
    } else if(self.defaultStyle == HNProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    switch (self.defaultMaskType) {
        case HNProgressHUDMaskTypeCustom:
        case HNProgressHUDMaskTypeBlack:{
            ...
            _backgroundView.backgroundColor = self.defaultMaskType == HNProgressHUDMaskTypeCustom ? self.backgroundLayerColor : [UIColor colorWithWhite:0 alpha:0.4];
            break;
        }
        case HNProgressHUDMaskTypeGradient:{
            if(!_backgroundRadialGradientLayer){
                _backgroundRadialGradientLayer = [HNRadialGradientLayer layer];
            }
...
- (UIWindow *)frontWindow {
    ...
    for (UIWindow *window in frontToBackWindows) {
        ...
        BOOL windowIHNisible = !window.hidden && window.alpha > 0;
        BOOL windowLevelSupported = (window.windowLevel >= UIWindowLevelNormal && window.windowLevel <= self.maxSupportedWindowLevel);
        
        if(windowOnMainScreen && windowIHNisible && windowLevelSupported) {
            return window;
        }
    }
#endif
...
- (void)addBlur
{
    UIBlurEffectStyle blurEffectStyle = self.defaultStyle == HNProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleExtraLight;
...
- (void)setDefaultAnimationType:(HNProgressHUDAnimationType)animationType {
